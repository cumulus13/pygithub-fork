...
~~~~~~~~~~~~~~~~~~~~
Core GitHubForker class with multi-authentication support.
...
9. **Multi-authentication** — supports forking from multiple owners using
...
class GitHubForker:
    """
    Production-ready GitHub repository forker with multi-authentication support.

    Parameters
    ----------
    client : github.Github or str or dict
        Authentication method. One of:
        - A PyGithub Github instance (single auth)
        - A GitHub token string (single auth)
        - A dict mapping owner names to tokens for multi-owner forking
        - An AuthManager instance for advanced multi-auth scenarios
    ...
    Quick start::
        ...
        # Automatically uses correct token per owner
        result1 = forker.fork("org-a/private-repo")
        result2 = forker.fork("org-b/private-repo")
    """

    def __init__(
        self,
        client: Union[Github, str, dict, AuthManager],
        config: Optional[ForkerConfig] = None,
    ) -> None:
        from .auth import AuthManager as _AuthManager
        
        # Handle different client types
        if isinstance(client, _AuthManager):
            self._auth_manager = client
            self._is_multi_auth = True
        elif isinstance(client, Github):
            # Single Github instance - wrap in AuthManager
            self._auth_manager = _AuthManager.__from_github_instance(client)
            self._is_multi_auth = False
        elif isinstance(client, str):
            # Single token string
            self._auth_manager = _AuthManager(client)
            self._is_multi_auth = False
            ...
            raise TypeError(
                "client must be Github instance, token string, auth dict, or AuthManager"
            )
    ...
    def _get_github_client(self, source: Union[str, Repository]) -> Github:
        """
        Get appropriate GitHub client for the source repository owner.
        
        In multi-auth mode, selects token based on repository owner.
        In single-auth mode, uses the default client.
        """
        if isinstance(source, Repository):
            owner = source.owner.login
        else:
            # Extract owner from "owner/repo" string
            parts = source.split("/")
            if len(parts) < 2:
                raise ValueError(f"Invalid source format: {source}")
            owner = parts[0]
        
        return self._auth_manager.get_client(owner)

    def _resolve_source(self, source: Union[str, Repository]) -> Repository:
        gh = self._get_github_client(source)
    ...
    def _find_existing_fork(
        ...
        # Get client for fork target (uses authenticated user's token)
        gh = self._auth_manager.get_client(self._get_target_login(organization))
        
        try:
            target_login = self._target_login(gh, organization)
        except GithubException:
            return None

        try:
            candidate = gh.get_repo(f"{target_login}/{repo.name}")
        except GithubException as exc:
            if exc.status == 404:
                return None
    ...
    def _get_target_login(self, organization: Optional[Union[str, Organization]]) -> str:
        """Get the login name of the fork target (org or user)."""
        if organization is not None:
            if isinstance(organization, Organization):
                return organization.login
            return str(organization)
        
        # Return None to indicate we should use authenticated user's token
        return "_authenticated_user_"

    def _target_login(
        self, gh: Github, organization: Optional[Union[str, Organization]]
    ) -> str:
    ...
    def _await_ready(self, result: ForkResult, start_time: float) -> None:
        ...
        while time.monotonic() < deadline:
            try:
                # Use authenticated user's client for fork lookup
                gh = self._auth_manager.get_client("_default_")
                refreshed = gh.get_repo(full_name)
